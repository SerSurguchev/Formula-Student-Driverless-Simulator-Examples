...
if not os.path.exists(config.PATH_WITH_IMAGES):
    os.makedirs(config.PATH_WITH_IMAGES)
...
while (t2 - t1) < config.TIME_LIM_SEC:
    ...
    [image] = client.simGetImages(
        [fsds.ImageRequest(camera_name='cam1', image_type=fsds.ImageType.Scene, pixels_as_float=False, compress=False)],
        vehicle_name='FSCar')
